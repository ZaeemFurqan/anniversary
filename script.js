...
img.onload=()=>{
    ...
    for(let r=0;r<rows;r++){
        for(let c=0;c<cols;c++){
            ...
            pieceImg.classList.add('puzzle-piece'); pieceImg.draggable=true;
            puzzleContainer.appendChild(pieceImg);
            pieces.push(pieceImg); correctOrder.push(pieceImg.src);
        }
    }
    shufflePieces(); initDragAndDrop();
};
...
function checkPuzzleComplete(){
    ...
    let complete=true;
    for(let i=0;i<correctOrder.length;i++){if(currentOrder[i]!==correctOrder[i]){complete=false;break;}}
    if(complete){
...
window.addEventListener('scroll',()=>{
    ...
    if(rect.top<window.innerHeight&&rect.bottom>=0){showLastPageMessage();}
},{once:true});
